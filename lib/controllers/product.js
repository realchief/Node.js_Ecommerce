...
module.exports = function(S){
  ...
  if (true || S.settings.environment.match(/worker/i)) S.instance.on('ready', function(){
  ...
  setTimeout(function(){
    return Async.waterfall([
      function(cb){
        S.instance.helpers.cache.LoadProductCategories(Belt.cw(cb, 0));
      }
    /*, function(cb){
        S.instance.helpers.cache.LoadProductOptions(Belt.cw(cb, 0));
      }*/
    , function(cb){
        S.log.warn('CATEGORIES: \n\n' + Belt.stringify(S.instance.categories));

        //S.log.warn('PRODUCT OPTIONS: \n\n' + Belt.stringify(S.instance.product_options));
