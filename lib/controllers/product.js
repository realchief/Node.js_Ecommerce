...
module.exports = function(S){
  ...
  if (S.settings.environment === 'production-worker-3')  S.instance.on('ready', function(){
    Async.forever(function(next){
      ...
      , function(cb){
          S.log.warn(gb.count + ' PRODUCTS TO DOWNLOAD AND DOWNSAMPLE');
      ...
      , function(cb){
          Async.eachSeries(gb.docs, function(d, cb2){
            S.log.warn('Downloading and downsampling product "' + d.get('name')
            + '" [' + d.get('_id').toString() + ']');
  ...
  if (S.settings.environment === 'production-worker-3' && Belt.get(S.instance, 'settings.notifications.platform_new_products_slack')) S.instance.on('ready', function(){
    Async.forever(function(next){
      var gb = {};
      ...
      , function(cb){
          Request({
            'url': S.instance.settings.notifications.platform_new_products_slack
          , 'method': 'post'
          , 'json': true
          , 'body': {
              'text': '*TOTAL SKUS*: ' + gb.total_count + ' available'
                    + (_.size(S.instance.vendor_ids) ? ('\n***\n' + _.map(_.values(S.instance.vendor_ids), function(v, i){
                        return '*' + v.name + '*: ' + gb.vendor_products[i] + ' SKUs available';
                      }).join('\n')) : '')
