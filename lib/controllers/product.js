...
module.exports = function(S){
  ...
  S.instance.express.all('/' + S.name + '/:_id/media/create.json', function(req, res){
    var a = {
      'o': {}
    }, self = S
     , gb = {};
    ...
    return Async.waterfall([
      function(cb){
        gb['filename'] = a.o.files.file.originalname.split('.');
        gb['extension'] = gb.filename.pop();
        gb.filename = gb.filename.join('.') + '-' + Belt.uuid() + '.' + gb.extension;

        return self.instance.helpers.s3.uploadPath({
          's3_path': self.name + '/' + req.params._id + '/media/' + gb.filename
        , 'path': Path.join(self.settings.__dirname, '/' + a.o.files.file.path)
        , 'content_type': a.o.files.file.mimetype
        }, Belt.cs(cb, gb, 'url', 1, 0));
      }
    , function(cb){
        return self.update({
          'query': a.o.query
