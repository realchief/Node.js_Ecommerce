...
module.exports = function(S){
    ...
    S.instance.express.all('/admin/' + S.name + '/:_id/sync.json', function(req, res){
      ...
      , function(cb){
          if (Belt.get(gb, 'doc.shopify.access_token')){
            return gb.doc.syncShopifyInventory(Belt.cw(cb));
          }

          if (Belt.get(gb, 'doc.woocommerce.consumer_key')){
            return gb.doc.syncWoocommerceInventory(Belt.cw(cb));
          }

          if (Belt.get(gb, 'custom_sync.strategy')){
            return gb.doc.syncCustomInventory(Belt.cw(cb));
          }
  ...
  if (true || S.settings.environment === 'production-worker-2' || S.settings.environment === 'production-worker-4') S.instance.on('ready', function(){
    var gb = {};
    Async.waterfall([
      function(cb){
        S.model.find({
