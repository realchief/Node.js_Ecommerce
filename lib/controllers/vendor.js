   ...
   */
    ...
    S.instance.express.all('/admin/' + S.name + '/:_id/sync.json', function(req, res){
      ...
      , function(cb){
          ...
          if (Belt.get(gb, 'doc.magento.url')){
    ...
    S.instance.express.all('/admin/' + S.name + '/export.csv', function(req, res){
      ...
      , function(cb){
          gb['objs'] = Belt.get(gb, 'docs.[].toCSVObject({"is_admin": true})');
          gb['fields'] = {};
          _.each(gb.objs, function(v, k){
            _.each(v, function(v2, k2){
              if (!_.isDate(v2) && (_.isObject(v2) || _.isArray(v2))) return;
              gb.fields[k2] = '';
            });
          });
        ...
        var csv = CSV.format({'headers': true})
                     .transform(function(doc, next){
                       doc = _.omit(doc, function(d){
                         return !_.isDate(d) && (_.isObject(d) || _.isArray(d));
                       });
    ...
    S.instance.express.all('/admin/' + S.name + '/signatures/export.csv', function(req, res){
      var a = {
        'o': _.extend({}, {
               'data': req.data()
             , 'session': req.session
      ...
      , function(cb){
          gb['objs'] = _.chain(gb.docs)
                        .reject(function(d){
                          return !Belt.get(d, 'shopify.access_token')
                              && !Belt.get(d, 'woocommerce.consumer_key');
                         })
                        .map(function(d){
                          return {
                            'vendor': d.name
                          , 'status': 'Signed agreement online on ' + Moment(d.created_at).format('MM-DD-YYYY HH:mm')
                          , 'signature_key': 'xxxx' + (Belt.get(d, 'shopify.access_token') || Belt.get(d, 'woocommerce.consumer_key')).slice(-4)
                          };
                        })
                        .value();
  ...
  setTimeout(function(){
    ...
    , function(cb){
        S.instance.helpers.cache.LoadVendors(function(){
          ...
          S.log.warn('Loaded ' + _.filter(S.instance.vendors || [], function(s){
            return Belt.get(s, 'magento.consumer_key') ? true : false;
