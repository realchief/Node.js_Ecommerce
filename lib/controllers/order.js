...
module.exports = function(S) {
  ...
  /**
   * @api {get} /order/cart/:cart/create.json Add Cart
   * @apiName Add Cart
   * @apiGroup Order
   ...
   */
   ...
   /**
     * @api {post} /order/:_id/product/:product/stock/update.json Update Product Stock
     * @apiName UpdateProduct
     * @apiGroup Order
     * @apiPermission admin
     *
   */
    S.instance.express.post('/admin/' + S.name + '/:_id/product/:product/stock/update.json', function(req, res){
      var a = {
        'o': _.extend({}, {
               'data': req.data()
             , 'session': req.session
      ...
      , function(cb){
          if (!gb.doc) return cb(new Error(S.name + ' not found'));
          gb.doc.new_prod = null;         
          var product = _.find(gb.doc.products, function (p) {            
            return p.product == req.params.product;           
          });

          var new_stock = _.find(product.source.product.stocks, function (stock) {      

            var old_stock = _.omit(a.o.data, ['_id', 'product', 'qty']);     
            var options = _.pick(stock.options, function (value, key, object) {
              return _.indexOf(_.keys(old_stock), key) > -1;
            });
            return _.every(_.keys(options), function (currentKey) {
              return options[currentKey]['value'] == old_stock[currentKey];
            });
          });
          if (!new_stock) {            
            cb(new Error('Not matched stock'));
          }
          if (new_stock['_id'] == product.source.stock['_id']) {
            console.log('Not changed Stock');
            cb(new Error('Not changed stock'));
          }
          gb.doc.new_stock = new_stock;
          gb.doc.product = product;
          cb();
        }
      , function(cb){          
        
          gb.doc.products = _.map(gb.doc.products, function (p) {
            if (p.product == req.params.product) {
                p['stock'] = gb.doc.new_stock._id;
                p['unit_price'] = gb.doc.new_stock.price;
                p['price'] = gb.doc.new_stock.price * a.o.data.qty;
                p['source']['stock'] = gb.doc.new_stock;
                p['options'] = _.omit(a.o.data, ['_id', 'product', 'qty']);
                p['source']['product']['options'] = _.omit(a.o.data, ['_id', 'product', 'qty']);
                p['quantity'] = parseInt(a.o.data.qty, 10);
                return p;
            }
            else return p;
          });
  ...
  /**
   * @api {get} /order/:_id/line_item/create.json Add Line Item
   * @apiName Add Line Item
   * @apiGroup Order
   * @apiPermission admin, public
   *
   */
  ...
  /**
   * @api {get} /order/:_id/product/:product/create.json Add Product
   * @apiName Add Product
   * @apiGroup Order
   * @apiPermission admin, public
   *
   */
    S.instance.express.get('/admin/' + S.name + '/:_id/product/:product/create.json', function(req, res) {
      var a = {
        'o': _.extend({}, {
               'data': req.data()
             , 'session': req.session
      ...
      return Async.waterfall([
        function(cb) {
          ...
          if (!gb.doc) return cb(new Error('order not found'));
          return S.instance.db.model('product').findOne({
            'slug': req.params.product
          }, Belt.cs(cb, gb, 'new_prod', 1));
        }, function(cb){
          if (gb.new_prod) return cb();
          return S.instance.db.model('product').findOne({
            '_id': req.params.product
          }, Belt.cs(cb, gb, 'new_prod', 1, 0));
        }, function (cb) {
          if (!gb.new_prod) return cb(new Error('product not found'));
          var passed_stocks = [];
          Async.during(
            function (cb2) {
              var next = true;
              if (gb.default_stock) next = false;
              return cb2(null, next);
            }, function (cb2) {
              var stock = null;
              _.find(gb.new_prod.configuration_array, function (stocks) {
                stock = _.find(stocks, function (stock) {
                  return _.indexOf(passed_stocks, stock.stocks[0]) < 0 && stock.available_quantity > 0;
                });
                if (stock) return true;
                return false;
              });
              if (!stock) return cb2(new Error('There is no available stock'));
              passed_stocks.push(stock.stocks[0]);
              return S.instance.db.model('stock').findOne({
                '_id': stock.stocks[0]
              }, Belt.cs(cb2, gb, 'default_stock', 1));
            }, function (err) {
              if (err) return cb(err);
              return cb();
            }
          ...
          if (!gb.doc) return cb(new Error('order not found'));
          gb.new_prod.populate('stocks', Belt.cs(cb, gb, 'prod', 1));
        }, function (cb) {
          var options =  _.mapObject(gb.default_stock.options, function(val, key) {
            return val['value'];
          });
          if (!gb.prod) return cb(new Error('Can not populate stocks'));
          var p = gb.prod.toObject();
          var prods = gb.doc.products;
          prods.push({
            'product': p._id
          , 'stock': gb.default_stock._id
          , 'options': options
          , 'sku': gb.default_stock.sku
          , 'unit_price': gb.default_stock.price
          , 'quantity': 1
          , 'price': gb.default_stock.price * 1
          , 'source': {
              'product': _.omit(p, [
                'base_configuration'
              , 'configurations'
   ...
   */
    ...
    S.instance.express.all('/admin/' + S.name + '/:_id/vendor/:vendor/order/:order/update.json', function(req, res){
      ...
      , function(cb){
          ...
          if (Belt.get(gb, 'vendor.shopify.access_token')){
          ...
          } else if (false && Belt.get(gb, 'vendor.magento')){
            ...
            self.instance.helpers.imap.FindStreetammoShippingEmail({
...
*/
  ...
  /**
   * @api {get} /order/:_id/products/delete.json Delete Products
   ...
   */
    ...
    S.instance.express.post('/admin/' + S.name + '/:_id/products/delete.json', function(req, res){
      var a = {
        'o': _.extend({}, {
               'data': req.data()
             , 'session': req.session
      ...
      return Async.waterfall([
        function(cb){
          console.log(a.o.data.products);
          if (!a.o.data.products) return cb(new Error('product is required'));
      ...
      , function(cb){
          if (!gb.doc) return cb(new Error('order not found'));
          var prods = _.filter(gb.doc.products, function(p) {         
            return _.indexOf(a.o.data.products, p.product.toString()) < 0;
          });         
          gb.doc.set({
            products: prods
          })
  ...
  /**
   * @api {get} /order/:_id/product/:product/delete.json Delete Product
