...
module.exports = function(S){
  ...
  S['sendOrderShippingNotification'] = function(options, callback){
    ...
    var html = S.instance.renderView(a.o.request, 'order_shipping_email', {
      'doc': Belt.get(a.o.doc, 'toSanitizedObject()')
    , 'message': 'Items in your order have shipped'
   ...
   */
    ...
    S.instance.express.all('/admin/' + S.name + '/:_id/vendor/:vendor/order/:order/update.json', function(req, res){
      ...
      , function(cb){
          ...
          Async.eachSeries(gb.doc.get('shipments') || [], function(s, cb2){
            if (s.sent_shipped_email) return cb2();

            s['sent_shipped_email'] = true;

            S.sendOrderShippingNotification({
              'doc': gb.doc
            , 'request': req
            , 'shipment': s
            }, Belt.cw(cb2));
    ...
    S.instance.express.all('/admin/' + S.name + '/:_id/shipments.json', function(req, res){
      var a = {
        'o': _.extend({}, {
               'data': req.data()
             , 'session': req.session
      ...
      , function(cb){
          ...
          gb['shipment'] = Belt.get(gb, 'doc.getShipments().0');

          if (!gb.shipment) return cb();

          var html = S.instance.renderView(req, 'order_shipping_email', {
            'doc': Belt.get(gb.doc, 'toSanitizedObject()')
          , 'message': 'Your order items have shipped'
          , 'status_update': 'Your order items have shipped'
          , 'tracking_link': gb.shipment.tracking_url ? '<a style="color:white;text-decoration:none;" href="' + gb.shipment.tracking_url + '">Track your shipment</a>' : ''
          , 'products': _.map(gb.shipment.products, function(p){
              return _.find(gb.doc.get('products'), function(p2){
                return p.toString() === p2._id.toString();
              });
            })
          ...
          S.instance.mailer.sendMail({
            'from': S.settings.email.order
          , 'to': 'ben@sack.io'//gb.doc.get('buyer.email')
          //, 'cc': S.settings.email.order
          , 'subject': 'Shipped! WANDERSET Order ' + gb.doc.get('slug')
          , 'text': html
          , 'html': html
          }, Belt.cw(cb, 0));
        }
        ...
        return res.status(200).json({
          'error': Belt.get(err, 'message')
        , 'data': Belt.get(gb, 'doc.getShipments()')
        });
      });
    });
   ...
   */
    ...
    S.instance.express.all('/admin/' + S.name + '/breakdown', function(req, res){
      var gb = {};
      ...
      , function(cb){
          gb['breakdown'] = S.model.orderBreakdown({
            'orders': gb.docs
          });
