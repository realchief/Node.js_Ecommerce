    ...
    if (Belt.get(S.settings, 'notifications.new_order_slack')){
      ...
      , 'body': {
          'text': 'New order '
                ...
                + ' <' + S.settings.host + '/admin/order/' + a.o.doc.get('_id').toString() + '/read>'
        , 'username': 'ORDER-BOT'
        , 'icon_emoji': ':moneybag:'
        }
      }, Belt.noop);
    ...
    if (Belt.get(S.settings, 'notifications.order_shipment_slack')){
      ...
      , 'body': {
          'text': 'New order shipment for Order #'
                + a.o.doc.get('slug') + ' from ' + Belt.get(S.instance.vendor_ids[a.o.shipment.vendor.toString()], 'name')
                + ' <' + S.settings.host + '/admin/order/' + a.o.doc.get('_id').toString() + '/read>'
        , 'username': 'ORDER-BOT'
        , 'icon_emoji': ':package:'
        }
   ...
   */
    S.instance.express.post('/' + S.name + '/create.json', function(req, res){
      ...
      , function(cb){
        ...
        if (err && Belt.get(S.settings, 'notifications.error')){
          Request({
            'url': S.settings.notifications.error
          , 'method': 'post'
          , 'json': {
              'order_error': err.message
            }
          }, Belt.np);
        }
      ...
      Async.forever(function(next){
        ...
        , function(cb){
            _.each(gb.docs, function(d){
              ...
              , 'body': {
                  'text': 'Order #'
                        + d.get('slug') + ' has unshipped items for more than three days!'
                        + ' <' + S.settings.host + '/admin/order/' + d.get('_id').toString() + '/read>'
                , 'username': 'ORDER-BOT'
