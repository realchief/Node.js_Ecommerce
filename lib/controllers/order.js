    ...
    S.instance.express.get('/admin/' + S.name + '/:_id/product/:product/create.json', function(req, res) {
      ...
      return Async.waterfall([
        function(cb) {
          ...
          if (!stock) return cb(new Error('There is no available stock'));
          return S.instance.model('stock').findOne({

          }, Belt.cs(cb, gb, 'default_stock', 1));
        }, function (cb) {
          if (!gb.default_stock) return cb(new Error('can not found default stock'));
          gb.doc.products.push({
            'product': gb.new_prod._id
          , 'stock': gb.default_stock._id
          , 'options': gb.default_stock.options
