...
*/
    ...
    S.instance.express.all('/' + S.name + '/:_id/v/:vendor/h/:hash/packing_slip.pdf', function(req, res){
      var a = {
        'o': _.extend({}, {
               'data': req.data()
             , 'session': req.session
      ...
      , function(cb){
          ...
          gb['pdf_path'] = S.instance.helpers.files.TempPath({
            'extension': 'pdf'
          });

          PDF.convertHTMLString(gb.html, gb.pdf_path, Belt.cw(cb, 0));
        }
      , function(cb){
          gb['success'] = true;
        ...
        if (gb.success) return res.sendFile(gb.pdf_path);

        res.status(404).send(err ? err.message : 'Forbidden');

        if (gb.pdf_path) FS.unlink(gb.pdf_path, Belt.np);
      });
    });
