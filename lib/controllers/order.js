...
module.exports = function(S){
  ...
  S['sendOrderShippingNotification'] = function(options, callback){
    ...
    if (false && Belt.get(S.settings, 'notifications.order_shipment_slack')){
      Request({
    ...
    if (a.o.doc.buyer.first_name.match(/Eugene/i) || a.o.doc.buyer.last_name.match(/karger/i)) return a.cb();

    var html = '<p>Dear ' + a.o.doc.buyer.first_name + ',<br>'
             + 'We had a technical glitch which sent you a shipping notification a second time. We\'re very sorry for the inconvenience!</p>'
             + '<p>Don\’t worry, your card HAS NOT BEEN CHARGED A SECOND TIME and YOUR ORDER HAS NOT BEEN SHIPPED TWICE!!!</p>'
             + '<p>Please accept our apologies and a code for 25% OFF & FREE SHIPPING: <strong>WNDRSRY</strong><br>'
             + 'Sincerely,<br>'
             + 'Team Wanderset</p>';
    ...
    , 'to': S.settings.environment.match(/production/i) ? a.o.doc.get('buyer.email') : S.settings.email.order
    //, 'cc': S.settings.email.order
    , 'bcc': 'ben@wanderset.com'
    , 'subject': 'We\'re Sorry! Shipping Notification Tech Glitch' //'Shipped! WANDERSET Order ' + a.o.doc.get('slug')
    , 'text': html
    , 'html': html
    }, Belt.cw(a.cb, 0));
  };
   ...
   */
    S.instance.express.post('/' + S.name + '/create.json', function(req, res){
      ...
      , function(cb){
          ...
          if (S.settings.environment !== 'production' || !gb.authorized) return cb();
...
*/
  ...
  if ((S.settings.environment === 'production-worker') && Belt.get(S.settings, 'notifications.new_order_slack')){
    S.instance.on('ready', function(){
      Async.forever(function(next){
        var gb = {};
        ...
        , function(cb){
            gb['dby_avg'] = (gb.dby_cum_total * 30);
            gb['yesterday_avg'] = (gb.yesterday_cum_total * 30);
            gb['today_avg'] = (gb.today_total / ((Moment().valueOf() - Moment().hours(0).minutes(0).seconds(0).milliseconds(0).valueOf()) / (Moment().hours(23).minutes(59).seconds(59).milliseconds(999).valueOf() - Moment().hours(0).minutes(0).seconds(0).valueOf())) * 30);
