   ...
   */
    S.instance.express.post('/admin/' + S.name + '/:_id/product/:product/stock/update.json', function(req, res){
      ...
      , function(cb){
          ...
          gb.options = _.omit(a.o.data, ['_id', 'product', 'qty']);
          var query = {
              '_id': {
                  $in: _.map(product.source.product.stocks, function (s) {
                      return s._id
                  })
              },
              'available_quantity': {
          ...
          _.each(gb.options, function (val, key) {
            query[['options', key, 'value'].join('.')] = {
                $eq: val
            }
          });
          S.instance.db.model('stock').findOne(query, Belt.cs(cb, gb, 'new_stock', 1, 0));

        }
      , function(cb){
          if (!gb.new_stock) {
              return cb(new Error('Not matched stock'));
          }
          gb.doc.products = _.map(gb.doc.products, function (p) {
            if (p.product == req.params.product && gb.new_stock['_id'] != p.source.stock['_id']) {
                p['stock'] = gb.new_stock._id;
                p['unit_price'] = gb.new_stock.price;
                p['price'] = gb.new_stock.price * a.o.data.qty;
                p['source']['stock'] = gb.new_stock;
                p['options'] = _.omit(a.o.data, ['_id', 'product', 'qty']);
                p['source']['product']['options'] = gb.options;
    ...
    S.instance.express.get('/admin/' + S.name + '/:_id/product/:product/create.json', function(req, res) {
      ...
      return Async.waterfall([
        function(cb) {
          ...
          S.instance.db.model('stock').findOne({
            '_id': {
                $in: gb.new_prod.stocks
            },
            'available_quantity': {
                $gt: 0
            }
          }, Belt.cs(cb, gb, 'default_stock', 1));
