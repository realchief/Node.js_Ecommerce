   ...
   */
    S.instance.express.post('/admin/' + S.name + '/:_id/product/:product/stock/update.json', function(req, res){
      ...
      , function(cb){
          ...
          var new_stock = _.find(product.stocks, function (stock) {
            var options = _.pick(stock.options, function (value, key, object) {
              return key in _.keys(a.o.data);
            });

            return _.chain(options).keys().every(function (currentKey) {
              return options[currentKey]['value'] == a.o.data[currentKey];
            });
          });
          ...
          if (!_.isMatch(new_stock, product.stock)) {
            gb.doc.new_prod = new_stock.product;
          }
        }
      , function(cb){
          if (!_.isNull(gb.doc.new_prod))
      ...
      , function(cb){
          if (_.isNull(gb.doc.new_prod)) cb();
          gb.doc.products = _.filter(gb.doc.products, function (p) {
            return p.product != req.params.product
          });
          var p = gb.doc.new_prod;
