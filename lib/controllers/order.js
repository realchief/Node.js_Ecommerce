   ...
   */
    S.instance.express.post('/' + S.name + '/create.json', function(req, res){
      ...
      , function(cb){
          ...
          if (Belt.get(gb, 'doc.recipient.postal_code') !== Belt.get(gb, 'doc.buyer.postal_code')){
            gb.doc.set({
              'support_status': 'possible fraud'
            , 'notes': 'Buyer and recipient postal codes do not match. Check with Ben before processing. Email customer to ask for copy of ID of billing contact plus copy of credit card used. Call phone numbers to inform customer that you will need this information to process order.'
            });
            gb['possible_fraud'] = true;
          }

          if (S.settings.environment !== 'production' || gb.possible_fraud) return cb();
