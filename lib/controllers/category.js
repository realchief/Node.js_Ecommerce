...
module.exports = function(S){
  ...
  S.instance.express.all('/' + S.name + '/count.json', function(req, res){
    ...
    return Async.waterfall([
      function(cb){
        return S.count(a.o, Belt.cs(cb, gb, 'count', 1, 0));
      }
    ], function(err){
  ...
  S.instance.express.all('/' + S.name + '/list.json', function(req, res){
    ...
    return Async.waterfall([
      function(cb){
        return S.list(a.o, Belt.cs(cb, gb, 'docs', 1, 0));
      }
    ], function(err){
      return res.status(200).json({
        'error': Belt.get(err, 'message')
  ...
  S.instance.express.all('/' + S.name + '/create.json', function(req, res){
    ...
    return Async.waterfall([
      function(cb){
        return S.create(a.o, Belt.cs(cb, gb, 'doc', 1, 0));
      }
    , function(cb){
  ...
  S.instance.express.all('/' + S.name + '/read.json', function(req, res){
    ...
    return Async.waterfall([
      function(cb){
        return S.read(a.o, Belt.cs(cb, gb, 'doc', 1, 0));
      }
    , function(cb){
        if (!gb.doc) return cb(new Error(S.name + ' not found'));
        return cb();
  ...
  S.instance.express.all('/' + S.name + '/:_id/update.json', function(req, res){
    ...
    return Async.waterfall([
      function(cb){
        return S.update(a.o, Belt.cs(cb, gb, 'doc', 1, 0));
      }
    , function(cb){
        if (!gb.doc) return cb(new Error(S.name + ' not found'));
        return cb();
  ...
  S.instance.express.all('/' + S.name + '/:_id/delete.json', function(req, res){
    ...
    return Async.waterfall([
      function(cb){
        return S.delete(a.o, Belt.cw(cb, 0));
      }
    ], function(err){
      return res.status(200).json({
        'error': Belt.get(err, 'message')
