...
module.exports = function(S){
  ...
  S['GetSessionCart'] = function(options, callback){
    ...
    , function(cb){
      ...
      if (err) S.instance.InstanceError(err, 'GetSessionCart', {
  ...
  S['AddPromoCode'] = function(options, callback){
    ...
    Async.waterfall([
      function(cb){
        ...
        gb['code'] = self.instance.promo_codes[a.o.code.toLowerCase().replace(/\W/g, '')];
        if (!gb.code) return cb(new Error('Promo code is invalid'));

        a.o.cart.line_items.push({
          'label': a.o.code + ' - ' + gb.code.label
        , 'details': {
            'promo_code': a.o.code.toLowerCase()
          }
        });
      ...
      if (err) S.instance.emit('error', err, 'AddPromoCode -' + a.o.code);
   ...
   */
    S.instance.express.post('/' + S.name + '/session/product/create.json', function(req, res){
      ...
      , function(cb){
        ...
        if (err) S.instance.emit('error', err, 'CreateCartSessionProduct' + (gb.doc ? ' - ' + Belt.get(gb.doc, '_id.toString()') : ''));
   ...
   */
    S.instance.express.all('/' + S.name + '/session/read.json', function(req, res){
      ...
      /*, function(cb){
        ...
        if (err) S.instance.emit('error', err, 'ReadCartSession' + (gb.doc ? ' - ' + Belt.get(gb.doc, '_id.toString()') : ''));
    ...
    S.instance.express.all('/admin/' + S.name + '/:_id/read.json', function(req, res){
      ...
      /*, function(cb){
        ...
        if (err) S.instance.emit('error', err, 'AdminReadCart - ' + req.params._id);
   ...
   */
    S.instance.express.all('/' + S.name + '/session/update.json', function(req, res){
      ...
      /*, function(cb){
        ...
        if (err) S.instance.emit('error', err, 'UpdateSessionCart' + (gb.doc ? ' - ' + Belt.get(gb.doc, '_id.toString()') : ''));
   ...
   */
    S.instance.express.all('/' + S.name + '/session/product/:product/delete.json', function(req, res){
      ...
      , function(cb){
        ...
        if (err) S.instance.emit('error', err, 'DeleteCartSessionProduct' + (gb.doc ? ' - ' + Belt.get(gb.doc, '_id.toString()') : ''));
   ...
   */
    S.instance.express.all('/' + S.name + '/session/product/:product/quantity/:quantity/update.json', function(req, res){
      ...
      , function(cb){
        ...
        if (err) S.instance.emit('error', err, 'UpdateCartSessionProductQuantity' + (gb.doc ? ' - ' + Belt.get(gb.doc, '_id.toString()') : ''));
   ...
   */
    S.instance.express.all('/' + S.name + '/session/promo_code/:code/create.json', function(req, res){
      ...
      /*, function(cb){
        ...
        if (err) S.instance.emit('error', err, 'CreateCartSessionPromoCode' + (gb.doc ? ' - ' + Belt.get(gb.doc, '_id.toString()') : ''));
