...
module.exports = function(S){
  ...
  /**
   * @api {post} /cart/product/create.json Add Product to Cart
   * @apiName CreateCartProduct
   * @apiGroup Cart
   * @apiPermission admin, current user
   *
   */
    S.instance.express.post('/' + S.name + '/product/create.json', function(req, res){
      var a = {
        'o': _.extend({}, {
               'data': req.data()
             , 'session': req.session
      ...
      , function(cb){
          var prods = _.filter(gb.doc.toSanitizedObject().products || [], function(p){
            return p.product !== a.o.data.product && !Belt.equal(p.options, a.o.data.options);
          });
  ...
  /**
   * @api {delete} /cart/product/:product/delete.json Delete Product from Session Cart
   * @apiName DeleteCartProduct
   * @apiGroup Cart
   * @apiPermission admin, current user
   *
   */
    S.instance.express.all('/' + S.name + '/product/:product/delete.json', function(req, res){
      var a = {
        'o': _.extend({}, {
               'data': req.data()
             , 'session': req.session
