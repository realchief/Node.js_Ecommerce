...
module.exports = function(Opts, Instance){
  ...
  M.method('uploadS3', function(options, callback){
    ...
    a.o = _.defaults(a.o, {
      's3_path': O.name + '/' + (self.get('slug') || self.get('_id').toString()) + '.' + (a.o.filename || a.o.path || a.o.url.replace(/(\?|#).*$/g, '')).split('.').pop()
    });
    a.o = _.defaults(a.o, {
  ...
  M.method('uploadDownsampleS3', function(options, callback){
    ...
    a.o = _.defaults(a.o, {
      's3_path': O.name + '/' + (self.get('slug') || self.get('_id').toString()) + '.downsample.' + (a.o.filename || a.o.url.replace(/(\?|#).*$/g, '')).split('.').pop()
    });
    a.o = _.defaults(a.o, {
      'mime': Mime.lookup(a.o.s3_path)
    });
  ...
  M.pre('save', function(next){
    ...
    console.log('Uploading media "' + self.get('_id').toString() + '" to S3...');
