...
module.exports = function(Opts, Instance){
  ...
  M.method('magentoOrderData', function(options, callback){
    ...
    gb['magento_vendors'] = _.pick(self.productsByVendor({
      ...
      return vend && Belt.get(vend, 'magento.consumer_secret');
    });
      ...
      gb.order = _.mapObject(gb.magento_vendors, function(v, k) {
        return _.extend(Belt.copy(gb.order), {
          'cart_items': _.map(v, function (v2) {
            ...
            return {
              'sku': Belt.get(v2, 'source.stock.source.record.sku')
              , 'qty': v2.quantity
              // , 'variant_id': Belt.get(v2, 'source.stock.source.record.id')
              , 'product_option': {
                'extension_attributes': {
                  'custom_options': _.map(Belt.get(v2, 'source.stock.options'), function (v3, k3) {
                    return {
                        'option_id': _.find(Belt.get(v2, 'source.product.source.record.options'), function (o) {
                            return o.title == v3.alias
                          }).option_id
                        , 'option_value': _.find(_.find(Belt.get(v2, 'source.product.source.record.options'), function (o) {
                          return o.title == v3.alias
                        }).values, function (val) {
  ...
  M.method('streetammoOrderData', function(options, callback){
    ...
    gb['order_lines'] = a.o.vendor ? _.map((self.productsByVendor({
      ...
      , 'totalprice': Belt.cast(Belt.cast(Belt.get(o, 'source.stock.source.record.sale_price') || Belt.get(o, 'source.stock.source.record.price'), 'string').replace(/\D/g, ''), 'number')
    ...
    , 'recipientstate': self.get('recipient.region')
