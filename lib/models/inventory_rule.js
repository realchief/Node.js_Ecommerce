...
module.exports = function(Opts, Instance){
  ...
  M.virtual('description').get(function(){
    ...
    if (this.get('product_hide')) astr += (astr ? ' and ' : '') + 'was hidden';
    if (this.get('product_show')) astr += (astr ? ' and ' : '') + 'was shown';
    if (this.get('product_brand')) astr += (astr ? ' and ' : '') + 'was rebranded to "' + this.get('product_brand') + '"';
    if (this.get('product_category')) astr += (astr ? ' and ' : '') + 'was categorized as "' + this.get('product_category') + '"';
  ...
  M.method('ProcessRule', function(options, callback){
    ...
    a.o.product.set({
      'hide_note': 'Product ' + self.get('description').join(' because ') + ' (Rule #' + self.get('_id').toString() + ')'
    });
