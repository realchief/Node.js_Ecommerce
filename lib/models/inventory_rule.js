  ...
  M.method('ProcessRule', function(options, callback){
    ...
    , function(cb){
        ...
        M.Instance.db.model('set').addProduct({
          'set': self.get('add_to_set')
        , 'product': a.o.product._id
        }, Belt.cw(cb, 0));
      }
    , function(cb){
        if (!self.get('remove_from_set')) return cb();

        M.Instance.db.model('set').removeProduct({
          'set': self.get('remove_from_set')
        , 'product': a.o.product._id
        }, Belt.cw(cb, 0));
      }
    , function(cb){
