...
module.exports = function(Opts, Instance){
  ...
  M.method('CheckProduct', function(options, callback){
    ...
    if (self.get('term')){
      ...
      if (_.every(self.get('term_fields'), function(f){
        return !(Belt.get(a.o.product, f) || '').match(gb.term);
      })) return false;
    }
  ...
  M.method('ProcessRule', function(options, callback){
    ...
    , 'hide_note': 'auto_category set to "' + self.get('product_category') + '" by inventory_rule ' + self.get('_id').toString()
    });

    if (self.get('product_hide')) a.o.product.set({
      'hide': true
    , 'hide_note': 'hidden by inventory_rule ' + self.get('_id').toString()
    });

    if (self.get('product_show')) a.o.product.set({
      'show': true
    , 'hide_note': 'shown by inventory_rule ' + self.get('_id').toString()
    });

    if (self.get('product_brand')) a.o.product.set({
      'manual_brand': self.get('product_brand')
    , 'hide_note': 'manual_brand set to "' + self.get('product_brand') + '" by inventory_rule ' + self.get('_id').toString()
    });
