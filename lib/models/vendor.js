...
module.exports = function(Opts, Instance){
  ...
  M['removeUnsyncedProducts'] = function (options, callback) {
    var a = Belt.argulint(arguments)
        , self = this
        , gb = {};
    a.o = _.defaults(a.o, {
    ...
    Async.waterfall([
      function(cb){
        ...
        }, Belt.cs(cb, a.o.gb, 'remove_products', 1, 0));
      }
      , function(cb){
        Async.eachSeries(a.o.gb.remove_products || [], function(e, cb2){
          e.set({
            'sync_hide': true
      ...
      , function(cb){
        ...
        }, Belt.cs(cb, a.o.gb, 'remove_stocks', 1, 0));
      }
      , function(cb){
        Async.eachSeries(a.o.gb.remove_stocks || [], function(e, cb2){
      ...
      , function(cb){
        Instance.db.model('set').find({
          'vendor': a.o.vendor.get('_id')
        }, Belt.cs(cb, a.o.gb, 'sets', 1, 0));
      }
      , function(cb){
        Async.eachSeries(a.o.gb.sets || [], function(e, cb2){
          var prods = _.filter(e.get('products') || [], function(p){
                return _.some(a.o.gb.products, function(p2){
                  return p2.toString() === p.toString();
                });
              }) || [];

          _.each(a.o.gb.products, function(p){
  ...
  M.method('toSanitizedObject', function(options, callback){
    ...
    if (!a.o.is_admin){
      ...
      Belt.delete(obj, 'magento.consumer_key');
      Belt.delete(obj, 'magento.consumer_secret');
      Belt.delete(obj, 'magento.verifier');
      Belt.delete(obj, 'magento.access_token');
      Belt.delete(obj, 'magento.access_secret');
    }
  ...
  M.method('syncMagentoInventory', function(options, callback){
    var a = Belt.argulint(arguments)
      , self = this
      , gb = {};
    a.o = _.defaults(a.o, {
    ...
    return Async.waterfall([
      function(cb){
        ...
        Instance.helpers.magento.IterateProducts({
          'vendor': {
            'url': self.get('magento.url')
            , 'oauth': {
              'consumer_key': self.get('magento.consumer_key')
              , 'consumer_secret': self.get('magento.consumer_secret')
              , 'verifier': self.get('magento.verifier')
              , 'token': self.get('magento.access_token')
              , 'token_secret': self.get('magento.access_secret')
            }
          }
        , 'progress_cb': function(p, cb2){
            Instance.helpers.magento.UpdateProduct({
              'product': p
            , 'vendor': self
            , 'last_sync': gb.last_sync
            , 'synced_at': gb.synced_at
    ...
    , function (cb) {
        M.removeUnsyncedProducts({gb: gb, vendor: self}, Belt.cw(cb));
      }
    ], function(err){
