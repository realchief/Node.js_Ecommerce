...
module.exports = function(Opts, Instance){
  ...
  MediaSchema.method('uploadS3', function(options, callback){
    ...
    a.o = _.defaults(a.o, {
      's3_path': O.name + '/media-' + self.get('_id').toString() + '.jpg' //+ (a.o.filename || a.o.path || a.o.url.replace(/(\?|#).*$/g, '')).split('.').pop()
    });
    a.o = _.defaults(a.o, {
  ...
  MediaSchema.pre('save', function(next){
    if (this.parent().get('skip_media_processing') || this.get('skip_processing') || this.get('super_downsampled')) return next();
