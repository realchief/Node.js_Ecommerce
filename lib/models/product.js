...
module.exports = function(Opts, Instance){
  ...
  MediaSchema.pre('save', function(next){
    ...
    if (this.parent().get('skip_media_processing') || this.get('skip_processing') || this.get('downloaded')) return next();
  ...
  MediaSchema.pre('save', function(next){
    if (this.parent().get('skip_media_processing') || this.get('skip_processing') || this.get('downsampled')) return next();
...
/*
    if (Belt.get(obj, 'label.us')){
      obj.label.us = Str.titleize(obj.label.us);
    }

    if (Belt.get(obj, 'brands.0')){
      obj.brands = _.map(obj.brands, function(b){
        return Str.titleize(b);
      });
    }
*/
...
*/
  ...
  if (Belt.get(Instance.settings, 'notifications.new_product_slack')){
    M.pre('save', function(next){
      ...
      Request({
        'url': Instance.settings.notifications.new_product_slack
      , 'method': 'post'
      , 'json': true
      , 'body': {
          'text': 'New product: '
    ...
    M.pre('save', function(next){
      if (!this.isModified('auto_hide')) return next();

      if (this.get('auto_hide')){
        Request({
          'url': Instance.settings.notifications.new_product_slack
        , 'method': 'post'
        , 'json': true
