...
//      'vendor': self.populated('vendor') ? self.get('vendor').toSanitizedObject(a.o) : obj.vendor
  ...
  M.pre('save', function(next){
    ...
    _.each(this.categories, function(c, i){
      this.categories[i] = c.replace(/^\W+|\W+$/g, '').replace(/\s+/g, ' ');
    });

    if (this.auto_category) this.auto_category = this.auto_category.replace(/^\W+|\W+$/g, '').replace(/\s+/g, ' ');
