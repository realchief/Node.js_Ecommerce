...
module.exports = function(Opts, Instance){
  var O = _.defaults(Belt.copy(Opts), {
    'no_media_processing': (Instance.settings.environment === 'production' ? true : Instance.settings.no_media_processing)
  });
  ...
  MediaSchema.pre('save', function(next){
    ...
    if (O.no_media_processing || M.Instance.settings.no_media_processing || this.parent().get('skip_media_processing') || this.get('skip_processing') || this.get('downloaded')) return next();
  ...
  MediaSchema.pre('save', function(next){
    if (O.no_media_processing || M.Instance.settings.no_media_processing || this.parent().get('skip_media_processing') || this.get('skip_processing') || this.get('downsampled')) return next();
  ...
  MediaSchema.pre('save', function(next){
    if (O.no_media_processing || M.Instance.settings.no_media_processing || this.parent().get('skip_media_processing') || this.get('skip_processing') || this.get('super_downsampled')) return next();
  ...
  , 'inventory_rules': {'type': Object, 'default': {}}
  });

  M.virtual('brand').get(function(){
    return this.get('manual_brand') || Belt.get(this, 'brands.0');
  });

  M.virtual('vendor_name').get(function(){
    return M.Instance.VendorName(this.get('vendor'));
  });
  ...
  M.method('getConfigurations', function(options, callback){
    ...
    _.each(self.get('stocks'), function(s){
      ...
      , 'options_array': _.values(s.options)
      };
    ...
    , 'on_sale': _.any(pobj, function(p){
        return p.compare_at_price;
      })
    , 'high_price': Belt.get(_.max(pobj, function(p){
        return p.price;
      }), 'price') || undefined
    , 'configuration_array': _.values(obj)
    });
...
//      'vendor': self.populated('vendor') ? self.get('vendor').toSanitizedObject(a.o) : obj.vendor
  ...
  M.pre('save', function(next){
    ...
    _.each(M.Instance.inventory_rules, function(r){
      if (r.CheckProduct({
        'product': self
      })) rules.push(r);
    });

    if (!_.any(rules)) return next();

    Async.eachSeries(rules, function(e, cb2){
      e.ProcessRule({
        'product': self
      }, Belt.cw(cb2));
    }, Belt.cw(next));
  ...
  M.pre('save', function(next){
    ...
    _.each(self.categories, function(c, i){
      self.categories[i] = c.replace(/^\W+|\W+$/g, '').replace(/\s+/g, ' ');
    });

    if (self.auto_category) self.auto_category = self.auto_category.replace(/^\W+|\W+$/g, '').replace(/\s+/g, ' ');
  ...
  if (Belt.get(Instance.settings, 'notifications.new_product_slack')){
    M.pre('save', function(next){
      ...
      , 'body': {
          'text': 'New product: '
                + (this.get('manual_brand') || this.get('brands.0'))
                + ' ' + this.get('label.us')
  ...
  M.pre('save', function(next){
    if (_.any(this.get('categories'))){
      var cat = _.find(this.get('categories'), function(c){
        return !_.some(M.Instance.categories_whitelist, function(w){
          return w === c;
        });
      });

      if (cat) return next(new Error('"' + cat + '" is not a valid category'));
    }

    if (this.get('auto_category')){
      var acat = this.get('auto_category');
      if (!_.some(M.Instance.categories_whitelist, function(w){
        return w === acat;
      })) return next(new Error('"' + acat + '" is not a valid category'));
    }
  ...
  M.post('save', function(){
    if (this.wasNew) M.Instance.db.model('set').addProduct({
      'set': 'new-arrivals'
    , 'product': this._id
    });
  });
  ...
  M.post('save', function(){
    if (this.low_price && this.compare_at_price && this.compare_at_price > this.low_price){
      M.Instance.db.model('set').addProduct({
        'set': 'on-sale'
      , 'product': this._id
      });

      M.Instance.db.model('set').removeProduct({
        'set': 'new-arrivals'
      , 'product': this._id
      });
    } else {
  ...
  M.post('save', function(){
    var brand = Belt.get(this, 'manual_brand');

    if (brand){
      M.Instance.db.model('set').addProduct({
        'set': Str.slugify(brand.toLowerCase())
      , 'product': this._id
      });
    }

    brand = Belt.get(this, 'brands.0');
