...
module.exports = function(S){
  ...
  S['SummarizeOrders'] = function(options, callback){
    ...
    , function(cb){
        ...
        gb.docs = Belt.get(gb, 'odocs.[].toSanitizedObject()');
        gb['product_docs'] = _.object(gb.products, _.map(gb.products, function(c){
          return _.filter(gb.docs, function(d){
            return _.some(d.products, function(p){
              return Belt.get(p, 'source.product.slug') === c;
            });
          });
        }));
        ...
        gb.docs = Belt.get(gb, 'odocs.[].toSanitizedObject()');
        gb['product_docs'] = _.object(gb.order_items, _.map(gb.order_items, function(c){
          return _.filter(gb.docs, function(d){
            return _.some(d.products, function(p){
              return Belt.get(p, '_id.toString()') === c;
            });
          });
        }));
    ...
    Async.waterfall([
      function(cb){
        gb['brand_sets'] = _.filter(S.instance.brand_sets, function(b){
          return !a.o.show_empties || Belt.get(v, 'filtered_products.length') < 4;
        });
