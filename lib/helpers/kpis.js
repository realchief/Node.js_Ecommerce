...
module.exports = function(S){
  ...
  S['DownloadMetrics'] = function(options, callback){
    ...
    , function(cb){
        ...
        gb['zip'] = new self.instance.zip();

        _.each(_.keys(gb.data), function(k, i){
          gb.zip.addFile(a.o.key + '.' + k + '.csv', gb.csvs[i].toString(), k + ' CSV data for ' + a.o.key);
        });
...
*/
  ...
  S.instance.express.all('/admin/' + S.name + '/:key/download.zip', function(req, res){
    ...
    , 'zip': true
      ...
      , 'Content-disposition': 'attachment; filename=' + req.params.key + '.zip'
      }).end(zip.toBuffer());
    });
  });
