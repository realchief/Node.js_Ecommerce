  ...
  S['SummarizeOrders'] = function(options, callback){
    ...
    , function(cb){
        ...
        _.each(gb.docs, function(d){
          var vendors = _.chain(d.products)
                         .map(function(p){ return Belt.get(p, 'source.product.vendor'); })
                         .compact()
                         .uniq(function(v){ return v.toString(); })
                         .value();
        ...
        gb['vendors'] = _.object(gb.vendors, _.map(gb.vendors, function(v){
          ...
          console.log('Vendor: ' + v + ' - ' + Belt.get(self.instance.vendor_ids[v], 'name'));
        ...
        _.each(gb.docs, function(d){
          _.each(d.products, function(v){
            var k = (Belt.get(v, 'source.product.brands.0.toLowerCase()') || '').replace(/\W/g, ' ');
        ...
        gb['category_docs'] = _.object(gb.categories, _.map(gb.categories, function(c){
          return _.filter(gb.docs, function(d){
            return _.some(d.products, function(p){
              var cat = Belt.get(p, 'source.product.categories.0.toLowerCase()') || Belt.get(p, 'source.product.auto_category.toLowerCase()');
              return cat === c;
            });
          })
        }));

        gb.category_docs = _.omit(gb.category_docs, function(v, k){
          return !Belt.get(v, 'length');
        });

        gb['count'] = 0;

        gb['categories'] = _.mapObject(gb.category_docs, function(v, k){
          console.log('Category metrics ' + (++gb.count) + ' of ' + _.size(gb.category_docs) + '...');
          console.log('Category: "' + k + '"');

          var docs = Belt.copy(v); //Belt.get(gb, 'odocs.[].toSanitizedObject()');

          var re = new RegExp('^' + self.instance.escapeRegExp(k));
        ...
        gb.options_docs = _.omit(gb.options_docs, function(v, k){
          return !Belt.get(v, 'length');
        });
        ...
        gb['product_options'] = _.mapObject(gb.options_docs, function(v, k){
          console.log('Size metrics ' + (++gb.count) + ' of ' + _.size(gb.options_docs) + '...');
          ...
          , 'product_filter': function(v2){
              return _.some(v2.options, function(v3){
                return v3.toLowerCase().replace(/\W/g, ' ') === k;
              });
            }
          });
        });
        ...
        gb.product_docs = _.omit(gb.product_docs, function(v, k){
          return !Belt.get(v, 'length');
        });
        ...
        gb['products'] = _.object(_.map(gb.product_docs, function(v, p){
          return Belt.arrayDefalse([Belt.get(p, 'brands.0'), Belt.get(p, 'label.us')]).join(' ')
               + ' (' + p.slug + ')';
        }), _.map(gb.product_docs, function(v, k){
          console.log('Product metrics ' + (++gb.count) + ' of ' + _.size(gb.products) + '...');
          ...
          return self.GetOrderMetrics({
            'docs': docs
          , 'product_filter': function(v2){
              return Belt.get(v2, 'source.product.slug') === k;
            }
          });
        }));
        ...
        gb['order_items'] = _.object(_.map(gb.order_items, function(p){
          ...
          var docs = Belt.copy(gb.product_docs[v._id]); //Belt.get(gb, 'odocs.[].toSanitizedObject()');
