...
/*    , function(cb){
        gb['order_number'] = Belt.get(gb, 'order.order.id');
        if (!gb.order_number) return cb();

        shopify.post('/admin/orders/' + gb.order_numer + '/transactions.json', {
          'transaction': _.extend({
            'amount': a.o.order.total_price
          , 'kind': 'sale'
          , 'gateway': 'wanderset'
          , 'currency': 'USD'
          , 'status': 'success'
