...
///////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  S.instance.express.all('/product/:_id', function(req, res, next){
    ...
    , function(cb){
      ...
      res.status(200).type('text/html').end(S.instance.renderView(req, 'product', {
        'doc': gb.doc
      , 'title': gb.title
      /*, 'configuration_options': req.query || {}
      , 'configuration': _.find(gb.doc.configurations, function(v, k){
          return v.available_quantity > 0
              && (!_.size(req.query) || _.every(req.query, function(v2, k2){
                   return
                 }))
        })*/
      , 'js_files': [
  ...
  S.instance.express.all('/', function(req, res){
    ...
    Async.waterfall([
      function(cb){
        gb['view'] = S.instance.renderView(req, 'homepage', {
          'doc': gb.doc
        , 'title': 'WANDERSET'
        });

        req.session['viewed_homepage'] = true;
      ...
      res.status(200).type('text/html').end(gb.view);
    });
  });
