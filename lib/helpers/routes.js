...
////////////////////////////////////////////////////////////////////////////////
  ...
  S['ConfigureSortParameters'] = function (sort) {
      if (!sort) sort = [];
  ...
  S['HasViewedView'] = function(req, view){
    if (req && req.session && req.session.views_viewed){
      req.session.views_viewed[view] = true;
    }
  };
  ...
  S['ListProducts'] = function(options, callback){
    ...
    a.o.data.sort = S.ConfigureSortParameters(a.o.data.sort);
    ...
    , function(cb){
        ...
        , 'limit': a.o.data.limit
        , 'populate': 'stocks'
        , 'sort': S.ConfigureSortParameters(a.o.data.sort)
        , 'skip': a.o.data.skip
...
/*        _.extend(gb.query, {
  ...
  S.instance.express.all('*', function(req, res, next){
    try {
      if (!(req.path || '').match(/\./)){
        ...
        req.session.routes.push(req.originalUrl);
        req.session.routes = req.session.routes.reverse().slice(0, 200).reverse();
        req.session['views_viewed'] = req.session.views_viewed || _.mapObject(S.instance.viewable.views, function(v, k){
          return false;
        });
      }
    } catch(e) {}
  ...
  S.instance.express.all('/.well-known/apple-developer-merchantid-domain-association', function(req, res){
    return res.sendFile(Path.join(S.settings.__dirname
    , S.settings.environment === 'production' ? '/resources/assets/apple-developer-merchantid-domain-association'
                                              : '/resources/assets/apple-developer-merchantid-domain-association.development'
    ));
  });

  if (S.settings.environment.match(/worker/i)){
    S.instance.express.all('*', function(req, res, next){
      var is_admin = S.instance.AdminLogin(req, res, next);
    });
  }
  ...
  S.instance.express.all(/\/products\/|\/set\/|\/brand\/|\/lifestyle/, function(req, res, next){
    if (req.originalUrl.match(/\.[^\.]+$/i)) return next();
...
///////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  S.instance.express.all('/checkout/complete', function(req, res){
    ...
    , function(cb){
      ...
      S.HasViewedView(req, 'order_confirmation');
  ...
  S.instance.express.all('/checkout', function(req, res){
    ...
    , function(cb){
        ...
        self.instance.helpers.order_rules.ProcessCartRules({
          'cart': gb.doc
        }, function(err, doc, rule_error){
          gb.doc = doc
          gb['rule_error'] = rule_error ? err : undefined;

          if (!rule_error && err) return cb(err);
    ...
    /*, function(cb){
      ...
      , 'page_type': 'cart'
      , 'total_value': gb.doc.total_price
      , 'error': Belt.get(gb, 'rule_error.message')
      , 'js_files': [
  ...
  S.instance.express.all('/set/:set', function(req, res, next){
    if (req.params.set.match(/\.(csv|json)$/)) return next();
  ...
  S.instance.express.all('/brand/:brand/:category/:subcategory', function(req, res, next){
    ...
    , function(cb){
      ...
      res.status(200).type('text/html').end(S.instance.renderView(req, 'set', {
        'doc': gb.doc
      , 'title': gb.title
      , 'sort': S.ConfigureSortParameters(req.data().sort) //|| '-created_at'
  ...
  S.instance.express.all('/lifestyle', function(req, res, next){
    ...
    Async.waterfall([
      ...
      S.HasViewedView(req, 'media_list');
  ...
  S.instance.express.all('/products', function(req, res, next){
    ...
    Async.waterfall([
      ...
      S.HasViewedView(req, 'product_list');
      ...
      , 'skip': Belt.get(a.o, 'data.skip')
      , 'limit': Belt.get(a.o, 'data.limit')
      , 'sort': S.ConfigureSortParameters(Belt.get(a.o, 'data.sort'))
  ...
  S.instance.express.all('/products/search', function(req, res, next){
    ...
    Async.waterfall([
      ...
      , 'page_title': gb.data.count ? '"' + gb.query.toUpperCase()  + '" Search Results'
                                    : 'No Search Results for "' + gb.query.toUpperCase() + '"'
      , 'prod_id': Belt.get((gb.data.docs || []).slice(0, 3), '[]._id.toString()')
      , 'page_type': 'searchresults'
      , 'js_files': [
          '/public/js/views/product_list.js'
  ...
  S.instance.express.all('/products/category/:category', function(req, res, next){
    ...
    Async.waterfall([
      function(cb){
        a.o.data['sort'] = S.ConfigureSortParameters(a.o.data.sort || '-created_at');
        a.o.data['q'] = {
          '$or': [
            {
              'categories': {
  ...
  S.instance.express.all('/page/:view', function(req, res){
    ...
    Async.waterfall([
      ...
      try {
        S.HasViewedView(req, req.params.view);
  ...
  S.instance.express.all('/order/:slug/vendor/:vendor_id/invoice.html', function(req, res){
      var a = {
          'o': _.extend({}, {
      ...
      Async.waterfall([
          function(cb){
              S.instance.db.model('order').findOne({
                  'slug': a.o.slug
          ...
          if (err){
              return res.status(400).end(err.message);
          } else if (gb.order === null) {
              return res.status(400).end('No such order');
          }

          try {
              res.status(200).type('text/html').end(S.instance.renderView(req, 'order_invoice_form', {
                  'title': 'WANDERSET'
                  , 'order': _.pick(gb.order, 'slug', 'created_at', 'source', 'recipient', 'products', 'line_items')
                  , 'vendor_id': a.o.vendor_id
              }));
          } catch(e){
  ...
  S.instance.express.all('/order/:slug/rma.html', function(req, res){
      var a = {
          'o': _.extend({}, {
              'slug': req.params.slug
          })
      ...
      Async.waterfall([
          ...
          try {
              res.status(200).type('text/html').end(S.instance.renderView(req, 'order_rma_form', {
                  'title': 'WANDERSET'
                  , 'order': _.pick(gb.order, 'slug')
              }));
          } catch(e){
