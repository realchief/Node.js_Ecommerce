...
module.exports = function(S){
  ...
  S['redis'] = S.settings.environment !== 'production-worker-4' ? S.instance.redis : null;
  ...
  if (true) S.instance.on('ready', function(){
    Async.forever(function(next){
      ...
      , 'brand_slugs'
        ...
        if (!S.redis) return cb2();

        S.log.warn('Reading from Redis "' + S.redis_prefix + e + '"...');

        S.redis.get(S.redis_prefix + e, function(err, val){
          var obj;
  ...
  if (true || S.settings.environment !== 'production') S.instance.on('ready', function(){
    Async.forever(function(next){
      return Async.waterfall([
        function(cb){
