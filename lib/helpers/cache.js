...
module.exports = function(S){
  S['redis_prefix'] = 'wanderset:' + S.settings.environment.replace(/\-worker[\-\d]*/, '') + ':';

  console.log('Set "redis_prefix" to "' + S.redis_prefix + '"...');

  //S['redis_prefix'] = S.settings.environment.match(/production/i) ? 'wanderset:production:' : 'wanderset:' + S.settings.environment + ':';
...
*/
  ...
  S['redis'] = true ? S.instance.redis : null;

  S['LoadCategories'] = function(options, callback){
    ...
    Async.waterfall([
      function(cb){
        ...
        Async.doWhilst(function(next){
          S.instance.db.model('category').find(a.o.query).sort(a.o.sort).skip(skip).limit(limit).exec(function(err, docs){
            if (err) return next(err);

            gb['docs'] = docs;

            _.each(docs, function(d){
              var cats = (Belt.get(d, 'label') || '').toLowerCase().split(' > ')
                , cur = gb.categories
                , c;
    ...
    , function(cb){
        ...
        , 'lifestyle': gb.categories.lifestyle
        };
        ...
        if (a.o.set_instance){
          ...
          self.instance['all_categories'] =  _.map(_.keys(Belt.objFlatten(S.instance.categories)), function(v){
            return v.replace(/\./g, ' > ');
          });
        }
  ...
  S['LoadProductCategories'] = function(options, callback){
    ...
    , function(cb){
        ...
        if (a.o.set_instance){
          self.instance['categories'] = gb.categories;
          self.instance['all_categories'] =  _.sortBy(_.map(_.keys(Belt.objFlatten(S.instance.categories)), function(v){
            return v.replace(/\./g, ' > ');
          }), function(v){
            return v;
          })
  ...
  S['LoadSets'] = function(options, callback){
    ...
    , function(cb){
        ...
        self.instance['brand_sets'] = _.filter(gb.sets, function(s){
          return !s.hide && s.brand; //&& Belt.get(s, 'filtered_products.length') >= 4;
        });
        ...
        self.instance['brand_logo_sets'] = Belt.arrayDefalse(_.sortBy(self.instance.brand_sets, function(d){
          if (Belt.get(d, 'filtered_products.length') < 4) return undefined;
    ...
    , function(cb){
        ...
        console.log('Setting "' + S.redis_prefix + 'sets" in Redis...');

        S.redis.set(S.redis_prefix + 'sets'
        , Belt.stringify(self.instance.sets)
        , Belt.cw(cb));
      }
    , function(cb){
        if (!S.redis) return cb();
  ...
  S.instance.express.all('/cache/product/whitelist/categories.json', function(req, res){
    res.status(200).json(S.instance.categories_whitelist);
  });
  ...
  S.instance.express.all('/admin/cache/vendor/list.json', function(req, res){
    res.status(200).json(S.instance.vendors);
  });
