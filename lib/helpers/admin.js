...
module.exports = function(S){
  S['proxy_host'] = (S.settings.environment === 'development' ? 'http://wanderset.com:9007'
                                                              : 'http://localhost:9007');

  S['admin_routes'] = [
    /\/admin\//
  , /^\/(product|set|media)\/.*(create|update|delete|remove|add)\.json/
  , /^\/(vendor)/
  , /^\/(stock)/
  ...
  S['sections'] = {
    'product': {
      ...
      , {
        ...
        , 'route': '/admin/product/list/invalid-categories'
        , 'view': 'admin_product_list'
        , 'js_files': [
            '/public/js/admin/product_list.js'
          ]
        , 'js_data': {
            'query': function(){
               return Belt.stringify({
                '$or': [
                  {
                    'categories': {
                      '$nin': Belt.copy(S.instance.categories_whitelist)
                    }
                  , 'categories.0': {
                      '$exists': true
                    }
  ...
  , 'inventory_rule': {
      ...
      , {
          'title': 'Invalid Categories'
        , 'route': '/admin/inventory_rule/list/invalid-categories'
        , 'view': 'admin_inventory_rule_list'
        , 'js_files': [
            '/public/js/admin/inventory_rule_list.js'
          ]
  ...
  , 'option_rule': {
      'pages': [
        {
          'title': 'Create'
        , 'route': '/admin/option_rule/create'
        , 'view': 'admin_option_rule'
        , 'button_title': 'Create'
        , 'js_files': [
            '/public/js/admin/option_rule_view.js'
          , '/public/js/admin/option_rule_create.js'
          ]
        }
      , {
          'title': 'List'
        , 'route': '/admin/option_rule/list'
        , 'view': 'admin_option_rule_list'
        , 'js_files': [
            '/public/js/admin/option_rule_list.js'
          ]
        , 'js_data': {
              'limit': 25
          }
      ...
      , {
          'title': 'Edit'
        , 'route': '/admin/option_rule/:_id/read'
        , 'view': 'admin_option_rule'
        , 'hide_menu': true
        , 'button_title': 'Update'
        , 'js_files': [
            '/public/js/admin/option_rule_view.js'
          , '/public/js/admin/option_rule_update.js'
          ]
        }
      , {
          'title': 'Edit'
        , 'route': '/admin/option_rule/:_id'
        , 'view': 'admin_option_rule'
        , 'hide_menu': true
        , 'button_title': 'Update'
        , 'js_files': [
  ...
  , 'vendor': {
      ...
      , {
          'title': 'Without Contacts'
        , 'route': '/admin/vendor/no-contacts/list'
        , 'view': 'admin_vendor_list'
        , 'js_files': [
            '/public/js/admin/vendor_list.js'
          ]
  ...
  , 'order': {
      ...
      , {
        ...
        , 'js_data': {
            'query': Belt.stringify({
              'products.source.order': {
                '$exists': true
              }
            , 'products.source.order.id': {
                '$exists': false
              }
            , 'products.source.order.request': {
                '$exists': false
              }
            , 'products.source.order.order.id': {
                '$exists': false
              }
            })
          }
  ...
  , 'kpi': {
      ...
      , {
          'title': 'Orders Same Day Last Week'
        , 'route': '/admin/kpi/order/same-day-last-week'
        , 'skip_route': true
        }
      , {
      ...
      , {
        ...
        , 'route': '/admin/kpi/order/this-year'
        , 'skip_route': true
        }
      , {
          'title': 'Orders Last Year'
        , 'route': '/admin/kpi/order/last-year'
        , 'skip_route': true
        }
      , {
          'title': 'Orders Lifetime'
        , 'route': '/admin/kpi/order/lifetime'
        , 'skip_route': true
        }
      , {
          'title': 'Products Active'
        , 'route': '/admin/kpi/product/active'
        , 'skip_route': true
        }
      ]
    }
...
/*
  if (S.settings.environment === 'development'){
    S.instance.express.use(/^\/admin/, Proxy({
      'target': S.proxy_host
    }));
  }
*/
  ...
  _.each(S.sections, function(v, k){
    _.each(v.pages, function(p){
      ...
      S.instance.express.all(p.route, function(req, res, next){
        ...
        , 'route': p.route
        , 'js_files': p.js_files
        , 'js_data': _.mapObject(p.js_data || {}, function(j){
            if (_.isFunction(j)) return j();
  ...
  S.instance.express.all('/admin/kpi/:model/:key', function(req, res){
    S.instance.redis.get('kpis:' + req.params.model + ':' + req.params.key, function(err, data){
      try {
      ...
      return res.status(200).type('text/html').end(S.instance.renderView(req, 'admin_kpis', {
        'section': 'KPIs'
      , 'title': Str.capitalize(Str.humanize(req.params.model)) + ' ' + Str.capitalize(Str.humanize(req.params.key))
      , 'route': '/admin/kpi/' + req.params.model + '/' + req.params.key
      , 'sections': S.sections
      , 'data': data
      , 'key': 'kpis:' + req.params.model + ':' + req.params.key
      }));
    });
  });
