...
module.exports = function(S){
  ...
  , 'content': {
      'pages': [
          ...
          , {
              'title': 'Edit'
              , 'route': '/admin/content/:page/update.json'
          }
      ]
  }
  };
  ...
  S.instance.express.all('/admin/kpi/order/today', function(req, res){
    S.instance.helpers.kpis.SummarizeOrders({
      'query': {
        'created_at': {
          '$gte': Moment().hours(0).minutes(0).seconds(0).milliseconds(0).toDate()
        , '$lte': Moment().hours(23).minutes(59).seconds(59).milliseconds(999).toDate()
        }
      }
    }, function(err, data){
  ...
  S.instance.express.all('/admin/kpi/order/today/email.json', function(req, res){
    res.status(200).json({
      'data': 'OK'
    });

    S.instance.helpers.kpis.EmailOrderSummary({
      'query': {
        'created_at': {
          '$gte': Moment().hours(0).minutes(0).seconds(0).milliseconds(0).toDate()
        , '$lte': Moment().hours(23).minutes(59).seconds(59).milliseconds(999).toDate()
    ...
    , 'email': req.data().email || S.settings.email.order
    , 'subject': 'Order KPIs - ' +  Moment().format('MM/DD/YYYY') + ' (Today)'
    }, function(err, data){

    });
  });

  S.instance.express.all('/admin/kpi/order/yesterday', function(req, res){
    S.instance.helpers.kpis.SummarizeOrders({
      'query': {
        'created_at': {
          '$gte': Moment().subtract(1, 'days').hours(0).minutes(0).seconds(0).milliseconds(0).toDate()
        , '$lte': Moment().subtract(1, 'days').hours(23).minutes(59).seconds(59).milliseconds(999).toDate()
        }
      }
    }, function(err, data){
      return res.status(200).type('text/html').end(S.instance.renderView(req, 'admin_order_kpis', {
  ...
  S.instance.express.all('/admin/kpi/order/yesterday/email.json', function(req, res){
    res.status(200).json({
      'data': 'OK'
    });
    ...
    , 'email': req.data().email || S.settings.email.order
    , 'subject': 'Order KPIs - ' +  Moment().subtract(1, 'days').format('MM/DD/YYYY') + ' (Yesterday)'
    }, function(err, data){

    });
  });

  S.instance.express.all('/admin/kpi/order/this-week', function(req, res){
    S.instance.helpers.kpis.SummarizeOrders({
      'query': {
        'created_at': {
          '$gte': Moment().days(0).hours(0).minutes(0).seconds(0).milliseconds(0).toDate()
        , '$lte': Moment().hours(23).minutes(59).seconds(59).milliseconds(999).toDate()
        }
      }
    }, function(err, data){
  ...
  S.instance.express.all('/admin/kpi/order/this-week/email.json', function(req, res){
    res.status(200).json({
      'data': 'OK'
    });
    ...
    , 'email': req.data().email || S.settings.email.order
    , 'subject': 'Order KPIs - ' + Moment().days(0).format('MM/DD/YYYY') + ' - ' +  Moment().format('MM/DD/YYYY') + ' (This Week)'
    }, function(err, data){

    });
  });

  S.instance.express.all('/admin/kpi/order/last-week', function(req, res){
    S.instance.helpers.kpis.SummarizeOrders({
      'query': {
        'created_at': {
          '$gte': Moment().subtract(7, 'days').days(0).hours(0).minutes(0).seconds(0).milliseconds(0).toDate()
        , '$lte': Moment().subtract(7, 'days').days(6).hours(23).minutes(59).seconds(59).milliseconds(999).toDate()
        }
      }
    }, function(err, data){
      return res.status(200).type('text/html').end(S.instance.renderView(req, 'admin_order_kpis', {
  ...
  S.instance.express.all('/admin/kpi/order/last-week/email.json', function(req, res){
    res.status(200).json({
      'data': 'OK'
    });
    ...
    , 'email': req.data().email || S.settings.email.order
    , 'subject': 'Order KPIs - ' + Moment().subtract(7, 'days').days(0).format('MM/DD/YYYY') + ' - ' +  Moment().subtract(7, 'days').days(6).format('MM/DD/YYYY') + ' (Last Week)'
    }, function(err, data){

    });
  });

  S.instance.express.all('/admin/kpi/order/this-month', function(req, res){
    S.instance.helpers.kpis.SummarizeOrders({
      'query': {
        'created_at': {
          '$gte': Moment().date(1).hours(0).minutes(0).seconds(0).milliseconds(0).toDate()
        , '$lte': Moment().hours(23).minutes(59).seconds(59).milliseconds(999).toDate()
        }
      }
    }, function(err, data){
  ...
  S.instance.express.all('/admin/kpi/order/this-month/email.json', function(req, res){
    res.status(200).json({
      'data': 'OK'
    });
    ...
    , 'email': req.data().email || S.settings.email.order
    , 'subject': 'Order KPIs - ' + Moment().format('MMMM YYYY') + ' (This Month)'
    }, function(err, data){

    });
  });

  S.instance.express.all('/admin/kpi/order/last-month', function(req, res){
    S.instance.helpers.kpis.SummarizeOrders({
      'query': {
        'created_at': {
          '$gte': Moment().subtract(1, 'months').date(1).hours(0).minutes(0).seconds(0).milliseconds(0).toDate()
        , '$lte': Moment().date(1).subtract(1, 'days').hours(23).minutes(59).seconds(59).milliseconds(999).toDate()
        }
      }
    }, function(err, data){
      return res.status(200).type('text/html').end(S.instance.renderView(req, 'admin_order_kpis', {
  ...
  S.instance.express.all('/admin/kpi/order/last-month/email.json', function(req, res){
    res.status(200).json({
      'data': 'OK'
    });
    ...
    , 'email': req.data().email || S.settings.email.order
    , 'subject': 'Order KPIs - ' + Moment().subtract(1, 'months').date(1).format('MMMM YYYY') + ' (Last Month)'
    }, function(err, data){
