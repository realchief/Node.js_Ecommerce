...
module.exports = function(S){
    ...
    S['CreateOrder'] = function(options, callback){
        ...
        Async.waterfall([
            ...
            function (cb) {
                // TODO check address information params in wanderset
                Request({
                    'url': a.o.vendor.url + S.api_urls.guest_cart + gb.cart_id + '/shipping-information'
                    , 'method': 'post'
                    , 'json': true
                    ,
                    'body': a.o.order.paymentInfo
                    , 'oauth': a.o.vendor.oauth
                }, function (err, res) {
                    cb();
                });
    ...
    S['ReadOrder'] = function(options, callback){
        ...
        Request({
            ...
            // TODO refactor all callback res -> data
            a.cb(err, data);
        });
    };
    ...
    S['UpdateProduct'] = function(options, callback){
        ...
        Async.waterfall([
            function(cb){
                // TODO products without images are not allowed?
            ...
            , function(cb){
                ...
                gb.doc.set({
                    ...
                    // TODO check Sanitize version --> TypeError: html.replace is not a function
                    , 'description': {
                        'us': gb.doc.get('description.us') || a.o.product.short_description || a.o.product.description
                    }
                    , 'vendor': a.o.vendor.get('_id')
                ...
                Async.eachSeries(a.o.product.variations || [], function(v, cb2){
                    ...
                    Async.waterfall([
                        ...
                        , function(cb3){
                            ...
                            var opts = {};
                            _.each(v.custom_attributes, function(o, k){
                                var attr = _.min(a.o.product.options, function(a){
                                    return Natural.LevenshteinDistance(
                                        a.title.toLowerCase()
                                        , o.attribute_code.toLowerCase()
                                    );
                                });
                            ...
                            gb2.stock.set({
                                ...
                                , 'available_quantity': v.extension_attributes.stock_item.is_in_stock && v.qty > 0 ? v.extension_attributes.stock_item.qty : 0
                            });
