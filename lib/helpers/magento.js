...
module.exports = function (S) {
    ...
    S['CreateOrderV1'] = function (options, callback) {
        ...
        S.LoginV1(a.o.vendor.magento.url, function (err, client) {
            ...
            client.login({
                'username': a.o.vendor.magento.username
                , 'apiKey': a.o.vendor.magento.api_key
            }, function (err, result) {
                ...
                Async.waterfall([
                    function (cb) {
                        ...
                        client.catalogProductCurrentStore({
                            ...
                            gb['store_id'] = WSDL ? data.storeView.$value : data.result;
                            cb(err)
                        });
                    }
                    , function (cb) {
                        client.shoppingCartCreate({
                            ...
                            gb.cart_id = WSDL ? data.quoteId.$value : data.result;
                            cb(err);
                        });
                    }
                    , function (cb) {
                        Async.eachSeries(a.o.order.cart_items, function (ci, cb2) {
                            ...
                            var products = [{
                                ...
                                , 'qty': ci.qty
                                , 'options': {
                                    'complexObjectArray': _.reduce(ci.product_option.extension_attributes.custom_options, function (m, o) {
                                        return m.concat([
                                            {
                                                'key': o.option_id
                            ...
                            if (WSDL) query['products'] = products;
                            else query['productsData'] = {
                                'complexObjectArray': products
                            };
                            client.shoppingCartProductAdd(
                                ...
                                if ((WSDL && data.result.$value != true) || (!WSDL && data.result != true)) return cb2(new Error('Cannot add product to cart'));
                                cb2(err);
                            });
                        }, function (err, data) {
                            cb(err);
                    ...
                    , function (cb) {
                        ...
                        if (WSDL) query['customer'] = addressData;
                        else {
                            query['customerAddressData'] = {
                                'complexObjectArray': addressData
                            };
                            query['store'] = gb.store_id;
                        }
                        client.shoppingCartCustomerAddresses(
                            query
                            , function (err, data) {
                            if (err || !data || data.body) return cb(new Error(data.body || 'No data returned from catalogProductList'));
                            if ((WSDL && data.result.$value != true) || (!WSDL && data.result != true)) return cb(new Error('Cannot add customer addresses to cart'));
                            cb(err);
                        });
                    }
                    , function (cb) {
                        client.shoppingCartShippingList({
                            ...
                            if (err || !data || data.body) return cb(new Error(data.body || 'No data returned from shoppingCartShippingList'));
                            if ((WSDL && !data.result.item) || (!WSDL && !data.result)) return cb(new Error('Shipping address not valid'));
                            gb.shipping_method = _.min(WSDL ? Array.isArray(data.result.item) ? data.result.item : [data.result.item] : data.result.complexObjectArray,
                                function (m) {
                                    return Belt.cast(WSDL ? m.price.$value : m.price, 'number');
                                });
                            cb(err);
                        });
                    }
                    , function (cb) {
                        ...
                        if (WSDL) query['method'] = gb.shipping_method.code.$value;
                        else query['shippingMethod'] = gb.shipping_method.code;
                        client.shoppingCartShippingMethod(
                            query
                            , function (err, data) {
                            if (!data || data.body) return cb(new Error(data.body || 'No data returned from catalogProductList'));
                            if ((WSDL && data.result.$value != true) || (!WSDL && data.result != true)) return cb(new Error('Cannot set shipping method to cart'));
                            cb(err);
                        });
                    }
                    , function (cb) {
                        client.shoppingCartPaymentList({
                            ...
                            if (!data || data.body) return cb(new Error(data.body || 'No data returned from catalogProductList'));
                            Async.eachSeries(WSDL ? Array.isArray(data.result.item) ? data.result.item : [data.result.item] : data.result.complexObjectArray, function (m, cb2) {
                                var query = {
                                    'quoteId': gb.cart_id
                                    , 'sessionId': gb.session
                                };
                                if (WSDL) query['method'] = {
                                    'method': m.code.$value
                                };
                                else query['paymentData'] = {
                                    'method': m.code
                                };
                                client.shoppingCartPaymentMethod(
                                    query
                                    , function (err, data) {
                                    if (err || !data || data.body || (WSDL && !data.result.$value) || (!WSDL && !data.result)) return cb2();
                                    gb.payment_set = true;
                                    cb(err);
                                });
                            }, Belt.cw(cb));
                    ...
                    , function (cb) {
                        ...
                        if (!gb.payment_set) return cb(new Error('Cannot set payment method to cart'));
                        client.shoppingCartOrder({
                            'quoteId': gb.cart_id
                            , 'sessionId': gb.session
                        }, function (err, data) {
                            if (err || !data || data.body) return cb(new Error(data.body || 'No data returned from catalogProductList'));
                            gb.order_number = WSDL ? data.result.$value : data.result;
                            cb(err);
                        });
                    }
                    , function (cb) {
                        ...
                        if (!WSDL) query['status'] = 'pending';
                        client.salesOrderAddComment(
                            query
                            , function (err, data) {
                            if (!data || data.body) return cb(new Error(data.body || 'No data returned from catalogProductList'));
                            if ((WSDL && data.result.$value != true) || (!WSDL && data.result != 1)) return cb(new Error('Cannot add comment to the order'));
                            cb(err);
                        });
                    }
                    , function (cb) {
                        Async.eachSeries(a.o.order.cart_items, function (ci, cb2) {
                            if (!ci.product_option.extension_attributes.custom_options ||
                                ci.product_option.extension_attributes.custom_options.length === 0) return cb2();
                            var query = {
                                'quoteId': gb.cart_id
                                , 'sessionId': gb.session
                                , 'orderIncrementId': gb.order_number
                            ...
                            client.salesOrderAddComment(
                                ...
                                , function (err, data) {
                                if (!data || data.body) return cb(new Error(data.body || 'No data returned from catalogProductList'));
                                if ((WSDL && data.result.$value != true) || (!WSDL && data.result != 1)) return cb(new Error('Cannot add options comment to the order'));
                                cb2(err);
                            });
                        }, Belt.cw(cb));
                    }
    ...
    S['CreateOrder'] = function (options, callback) {
        ...
        if (a.o.vendor.name.toLowerCase().indexOf('boy london') != -1 || a.o.debug) return a.cb();
        ...
        Async.waterfall([
            ...
            function (cb) {
                var cheapestShipping = _.min(gb.shipping_methods, function (m) {
                    return m.price_incl_tax;
                });
    ...
    S['ReadOrder'] = function (options, callback) {
        ...
        if (a.o.vendor.magento.version == 1) {
            ...
            S.LoginV1(a.o.vendor.magento.url, function (err, client) {
                ...
                client.login({
                    ...
                    client.salesOrderInfo({
                        'sessionId': gb.session
                        , 'orderIncrementId': a.o.order
                    }, function (err, data) {
                        if (!data || data.body) return a.cb(new Error(data ? data.body : 'No data returned from salesOrderInfo'));
            ...
            if (!gb.vendor_url) return a.cb(new Error('vendor not found'));
            ...
            Request({
                  'url': gb.vendor_url + S.api_urls.orders + a.o.order
                , 'method': 'get'
                , 'json': true
                , 'oauth': gb.oauth
            }, function (err, data) {
    ...
    S['UpdateProduct'] = function (options, callback) {
        ...
        Async.waterfall([
            ...
            , function (cb) {
                ...
                a.o.product.description = a.o.product.description ? a.o.product.description : _.find(a.o.product.custom_attributes, function (attr) {
                    return attr.attribute_code === 'description'
                }) || {};
                a.o.product.short_description = a.o.product.short_description ? a.o.product.short_description : _.find(a.o.product.custom_attributes, function (attr) {
                    return attr.attribute_code === 'short_description'
                }) || {};
                ...
                gb.doc.set({
                    ...
                    !_.any(a.o.product.images) ? true : gb.doc.get('hide') || false
                    , 'options': _.object(
            ...
            , function (cb) {
                ...
                Async.eachSeries(a.o.product.variations, function (v, cb2) {
                    ...
                    Async.eachSeries(optionsPermutationsArray || [{}], function (opts, cb3) {
                        ...
                        if (a.o.vendor.magento.url.indexOf('boy-london') !== -1) price *= S.instance.GBPtoUSD();
                        ...
                        Async.waterfall([
                            function (cb4) {
                                self.instance.db.model('stock').findOne({
                                    'sku': Belt.cast(v.sku + (_.keys(opts).length ? '-' + _.map(opts, function (opt) {
                                        return opt.alias + opt.value
                                    }).join('-') : ''), 'string')
                            ...
                            , function (cb4) {
                                ...
                                gb2.stock.set({
                                    ...
                                    , 'available_quantity': v.extension_attributes && v.extension_attributes.stock_item.is_in_stock &&
                    ...
                    , function (cb) {
                        ...
                        Async.forever(function (next) {
                            ...
                            client.catalogProductList({
                                        ...
                                        'complexObjectArray': [
                                            {
                                                ...
                                                , 'value': {
                                                    'key': 'lt'
                                                    , 'value': S.FormatDate(new Date(today - 70 * (gb.page - 1) * 24 * 60 * 60 * 1000))
                                                }
                                            }
                                            , {
                                                'key': 'created_at'
                                                , 'value': {
                                                    'key': 'gt'
                                                    , 'value': S.FormatDate(new Date(today - 70 * gb.page * 24 * 60 * 60 * 1000))
                                                }
                                            }
                                            , {
                                                'key': 'type'
                                ...
                                if (err || !data || data.body) return cb(new Error(err || (data ? data.body : 'No data returned from catalogProductList')));
                                if ((WSDL && !data.storeView.item) || (!WSDL && !data.result)) return next();
                                if (WSDL && !Array.isArray(data.storeView.item)) data.storeView.item = data.storeView.item ? [data.storeView.item] : [];

                                gb['products'] = WSDL ? _.filter(data.storeView.item, function (i) {
                                        return i.type.$value != 'virtual';
                                    }) :
                                    _.filter(data.result.complexObjectArray, function (i) {
                                        return i.type != 'virtual'
                                    });
                                ...
                                Async.eachSeries(gb.products, function (p, cb2) {
                                    Async.waterfall([
                                                ...
                                                , function (err, data) {
                                                    ...
                                                    if (WSDL) {
                                                        ...
                                                        _.each(!data.result ? [] : Array.isArray(data.result.item) ? data.result.item : [data.result.item], function (s) {
                                                            p.stocks.push(_.mapObject(s, function (v, k) {
                                                    ...
                                                    p.extension_attributes = {
                                                        'stock_item': {
                                                            'is_in_stock': _.any(_.filter(p.stocks, function (s) {
                                                                return s.is_in_stock;
                                                            }))
                                                            , 'qty': _.reduce(p.stocks, function (m, v) {
                                                                return m + Belt.cast(v.qty, 'number');
                                                            }, 0)
                                                        }
                                                    };
                                                    cb3(err)
                                        ...
                                        , function (cb3) {
                                            var sku = WSDL ? p.sku.$value : p.sku;
                                            if (sku.indexOf('TVK-') === 0) sku = sku.substring(0, sku.length - 4) + (Belt.cast(sku.substring(sku.length - 4), 'number') + 1);
                                            client.catalogProductList({
                                                'sessionId': gb.session
                                                , 'storeView': gb.store_id
                                                , 'filters': {
                                                ...
                                                if ((WSDL && !data.storeView.item) || (!WSDL && !data.result)) return cb3();
                                                if (WSDL && !Array.isArray(data.storeView.item)) data.storeView.item = data.storeView.item ? [data.storeView.item] : [];
                                                var children = WSDL ? data.storeView.item : data.result.complexObjectArray;
                                                p.children = [];

                                                Async.eachSeries(children, function (c, cb4) {
                                                    Async.waterfall([
                                                        function (cb5) {
                                                            client.catalogProductInfo({
                                                                'sessionId': gb.session
                                                                , 'productId': WSDL ? c.product_id.$value : c.product_id
                                                            }, function (err, data) {
                                                                if (!data || data.body) return cb5(new Error(data ? data.body : 'No data returned from catalogProductInfo'));
                                                                c = WSDL ? data.info : data.result;
                                                                cb5(err)
                                                            });
                                                        }
                                                        , function (cb5) {
                                                            var query = {'sessionId': gb.session};
                                                            if (WSDL) query['products'] = [c.product_id.$value];
                                                            else query['productIds'] = {"complexObjectArray": [c.product_id]};
                                                            client.catalogInventoryStockItemList(
                                                                query
                                                                , function (err, data) {
                                                                    if (err || !data || data.body) return cb5(new Error(err || (data ? data.body : 'No data returned from catalogInventoryStockItemList')));
                                                                    if (WSDL) {
                                                                        c['stocks'] = [];
                                                                        _.each(!data.result ?
                                                                            [] : Array.isArray(data.result.item) ?
                                                                                data.result.item : [data.result.item], function (s) {
                                                                            c.stocks.push(_.mapObject(s, function (v, k) {
                                                                                return v.$value ? v.$value : v;
                                                                            }));
                                                                        })
                                                                    } else {
                                                                        c['stocks'] = data.result.complexObjectArray;
                                                                    }
                                                                    c.extension_attributes = {
                                                                        'stock_item': {
                                                                            'is_in_stock': _.any(_.filter(c.stocks, function (s) {
                                                                                return s.is_in_stock;
                                                                            }))
                                                                            , 'qty': _.reduce(c.stocks, function (m, v) {
                                                                                return m + Belt.cast(v.qty, 'number');
                                                                            }, 0)
                                                                        }
                                                                    };
                                                        ...
                                                        , function (cb5) {
                                                            var query = {'sessionId': gb.session};
                                                            if (WSDL) query['product'] = c.product_id.$value;
                                                            else query['productId'] = c.product_id;
                                                            client.catalogProductAttributeMediaList(
                                                                query
                                                                , function (err, data) {
                                                                    if (!data || data.body) return cb5(new Error(data ? data.body : 'No data returned from catalogProductInfo'));
                                                                    ...
                                                                    c.images = [];
                                                                    if (!data.result || (WSDL && !data.result.item)) return cb5();
                                                                    else if (WSDL && !Array.isArray(data.result.item)) data.result.item = [data.result.item];
                                                                    c.images = _.map(WSDL ? data.result.item : data.result.complexObjectArray, function (m) {
                                                                        return {
                                                                            'src': WSDL ? m.url.$value : m.url
                                                                            , 'label': WSDL ? m.label.$value : m.label
                                                                            , 'file': WSDL ? m.file.$value : m.file
                                                        ...
                                                        var sku = WSDL ? c.sku.$value : c.sku;
                                                        var values = sku.split('-');
                                                        values = values.slice(values.length - (sku.indexOf('TVK') === 0 ? 2 : 1));
                                                        c.options = {};
                                                        if (!Array.isArray(p.variations)) p.variations = [];
                                                        if (Array.isArray(p.options)) p.options = {};
                                                        _.each(values, function (o, i) {
                                                            // FIXME HARDCODED for Tavik and Boy London
                                                            var option = values.length === 1 ? 'Size' :
                                                                i === 0 ? 'Size' :
                                                                    i === 1 ? 'Color' :
                                                                        'OPTION ' + (i + 1);
                                                            if (!Array.isArray(c.variation_config_options)) c.variation_config_options = [];
                                                            c.variation_config_options.push({
                                                                'title': option
                                                                , 'value': o
                                                            });
                                                        });
                                                        if (c.images && c.images.length > 0) p.images = p.images.concat(_.filter(c.images, function (i) {
                                                            return _.findIndex(p.images, function (i2) {
                                                                return i.src != i2.src;
                                                            }) == -1;
                                                        }));

                                                        if (WSDL) {
                                                            c = _.mapObject(c, function (v, k) {
                                                            ...
                                                            c = JSON.parse(JSON.stringify(c).replace(/\$value/g, 'value'));
                                                        }
                                                        p.variations.push(Belt.copy(c));
                                                        p.children.push(Belt.copy(c));
                                                        cb4(err);
                                                    });
                                                }, Belt.cw(cb3));
            ...
            function (cb) {
                ...
                Async.forever(function (next) {
                    ...
                    Request({
                        ...
                        gb.products = _.filter(gb.products, function (i) {
                            return i.type_id !== 'virtual' && i.visibility !== 1;
                        });
                                            ...
                                            Request({
                                                ...
                                                if (data && data.body) {
                                                    _.each(po.values, function (option) {
                                                        option.title = _.find(data.body.options, function (v) {
                                                            return option.value_index == v.value
                                                        }).label;
                                                        // TODO TEMP
