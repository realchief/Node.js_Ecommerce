                ...
                Async.waterfall([
                    function(cb){
                        ...
                        client.catalogProductList({
                            ...
                            if (!Array.isArray(data.storeView.item)) data.storeView.item = [data.storeView.item];
                            // gb['products'] = _.filter(data.storeView.item, function (i) { return i.type.$value != 'virtual'; }) || [];
                                 ...
                                 Async.waterfall([
                                    function (cb3) {
                                        client.catalogProductInfo({
                                            'sessionId': gb.session
                                            , 'productId': p.product_id.$value
                                        }, function (err, data) {
                                            if (!data || data.body) return cb3(new Error(data.body || 'No data returned from catalogProductInfo'));
                                            p = data.info;
                                            cb3(err)
                                        });
                                    }
                                    , function (cb3) {
                                        client.catalogProductListOfAdditionalAttributes({
                                            'sessionId': gb.session
                                            , 'productId': p.product_id.$value
                                        }, function (err, data) {
                                            if (!data || data.body) return cb3(new Error(data.body || 'No data returned from catalogProductInfo'));
                                    ...
                                    , function (cb3) {
                                        client.catalogInventoryStockItemList({
                                            'sessionId': gb.session
                                            , 'products': [p.sku.$value]
                                        }, function (err, data) {
                                            if (!data || data.body) return cb3(new Error(data.body || 'No data returned from catalogInventoryStockItemList'));
                                            p['stocks'] = [];
                                    ...
                                    , function (cb3) {
                                        ...
                                        client.catalogProductCustomOptionList({
                                            ...
                                            Async.eachSeries(data.result.item, function(o, cb4){
                                                client.catalogProductCustomOptionInfo({
                                                    'sessionId': gb.session
                                                    , 'optionId': o.option_id.$value
                                                }, function (err, data) {
                                                    if (!data || data.body) return cb4(new Error(data.body || 'No data returned from catalogProductCustomOptionInfo'));
                                                    data.result['option_id'] = o.option_id.$value;
                                    ...
                                    , function (cb3) {
                                        client.catalogProductAttributeMediaList({
                                            'sessionId': gb.session
                                            , 'product': p.product_id.$value
                                        }, function (err, data) {
                                            if (!data || data.body) return cb3(new Error(data.body || 'No data returned from catalogProductInfo'));

                                            if (!data.result.item) return cb3();
                                            else if (!Array.isArray(data.result.item)) data.result.item = [data.result.item];
                                            p.images = _.map(data.result.item, function(m) {
                                                return {
                                                    'src': m.url.$value
                                                    , 'label': m.label.$value
                                                    , 'file': m.file.$value
                                                    , 'position': m.position.$value
                                                    , 'types': _.map(Array.isArray(m.types.item) ? m.types.item : [m.types.item], function(t) {
            ...
            function(cb){
                ...
                Async.forever(function(next){
                    ...
                    Request({
                        ...
                        productsJSON.items = _.filter(productsJSON.items, function (i) { return i.type_id !== 'virtual' && i.visibility !== 1; });
                                         ...
                                         Request({
                                             ...
                                             _.each(p.options, function (o) {
                                                 _.each(data.body.custom_attributes, function (ca) {
                                                     if (ca.attribute_code == o.title.toLowerCase()) {
