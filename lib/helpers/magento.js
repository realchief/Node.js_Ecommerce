...
module.exports = function(S){
    ...
    S['IterateProductsV1'] = function(options, callback){
        ...
        console.log('ide6', a.o.vendor.magento.url);
        // http://159.65.61.200
        // Wanderset
        // somerandomapikey
        // https://www.criminaldamage.co.uk
        // wanderset-cd
        // wanderset123
        S.LoginV1('https://www.boy-london.com', function(err, client) {
            console.log('ide7');
            client.login({'username': 'wanderset-boy', 'apiKey': 'Wanderset123'}, function(err, result) {
        /*S.LoginV1('http://159.65.61.200', function(err, client) {
            console.log('ide7');
            client.login({'username': 'Wanderset', 'apiKey': 'somerandomapikey'}, function(err, result) {*/
                console.log('ide8', err, JSON.stringify(client.wsdl._includesWsdl.length));
                ...
                Async.waterfall([
                    function(cb){
                        ...
                        client.catalogProductList({
                            ...
                            //console.log('Got products', JSON.stringify(data.result.complexObjectArray[0]));
                            if (!data || data.body) return cb(new Error(data ? data.body : 'No data returned from catalogProductList'));
                            if (WSDL && !Array.isArray(data.storeView.item)) data.storeView.item = [data.storeView.item];
                            gb['products'] = WSDL ? _.filter(data.storeView.item, function (i) { return i.type.$value != 'virtual'; }) || [] :
                                _.filter(data.result.complexObjectArray, function(i) { return i.type != 'virtual' }).slice(3000, 3100);

                            //gb.products = [data.result.complexObjectArray[0]];
                            /*if (!_.any(gb.products) || gb.lastItemId === _.last(gb.products).product_id.$value) {
                                cb();
                                return; // stops the forever loop
                            } else {
                                gb.lastItemId = _.last(gb.products)[a.o.magento_version == 1 ? 'entity_id' : 'id'];
                            }*/
                            console.log('GOT PRODUCTS LIST:', gb.products.length);


                             Async.eachSeries(gb.products, function(p, cb2){
                                //console.log('Getting product', p.product_id);
                                 Async.waterfall([
                                    function (cb3) {
                                        client.catalogProductInfo({
                                            ...
                                            // else if (data.info.visibility.$value !== 1) return cb2();
                                            p = WSDL ? data.info : data.result;
                                            //console.log('PRODUCT:', p.name, p.product_id, err);
                                            cb3(err)
                                        });
                                    }
                                    , function (cb3) {
                                    ...
                                    , function (cb3) {
                                        ...
                                        client.catalogInventoryStockItemList(
                                            query
                                            , function (err, data) {
                                                //console.log('STOCKS:', JSON.stringify(data));
                                                ...
                                                //console.log('-----', p.extension_attributes);
                                                cb3(err)
                                        });
                                    }
                                    , function (cb3) {
                                            ...
                                            , function (err, data) {
                                                ...
                                                Async.eachSeries(WSDL ? data.result.item : data.result.complexObjectArray, function(o, cb4){
                                                    client.catalogProductCustomOptionInfo({
                                                        ...
                                                        //console.log('CUSTOM OPTIONS:', JSON.stringify(data));
                    ...
                    // TODO remove products with availablity == '1'
