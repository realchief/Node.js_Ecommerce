...
module.exports = function(S){
  ...
  if (S.settings.environment === 'production-worker-2') S.instance.on('ready', function(){
    Async.forever(function(next){
      ...
      , function(cb){
          if (!gb.doc) return cb(new Error('Nike set not found'));

          S.instance.db.model('product').find({
            'brands': /nike/i
          }, {'_id': 1}, Belt.cs(cb, gb, 'products', 1, 0));
        }
      , function(cb){
          gb['products'] = _.pluck(gb.products, '_id');
          gb['products'] = _.filter(gb.products, function(p){
            return !_.some(gb.doc.products, function(p2){
              return p2.toString() === p.toString();
            });
          });

          gb.doc.set({
            'products': (gb.doc.get('products') || []).concat(gb.products)
          });

          gb.doc.save(Belt.cw(cb, 0));
        }
      ], function(err){
        if (err) S.emit('error', err);
  ...
  if (S.settings.environment === 'development') S.instance.on('ready', function(){
    Async.forever(function(next){
      var gb = {};
      ...
      , function(cb){
          Async.eachLimit(_.shuffle(gb.docs), 5 || OS.cpus().length, function(d, cb2){
            d.syncShopifyInventory(Belt.cw(cb2));
          }, Belt.cw(cb));
        }
      ], function(err){
